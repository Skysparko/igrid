...
const CAPABILITY_ICONS: LucideIcon[] = [Palette, Globe, ShieldCheck, Zap, Building2, Headphones];
...
export default function WhiteLabel({ content }: WhiteLabelProps) {
  const tag = content?.whiteLabelTag ?? "White Label Solution";
  const title = content?.whiteLabelTitle ?? "Launch Your Own LMS Platform — Powered by iGird";
  const description =
    content?.whiteLabelDescription ??
    "License the full iGird platform under your own brand. We provide the technology; you own the experience. Perfect for EdTech startups, training companies, and enterprises building internal academies.";

  const capabilities =
    content?.whiteLabelCards && content.whiteLabelCards.length > 0
      ? content.whiteLabelCards.map((card) => ({
  ...
  const bigCardTitle =
    content?.whiteLabelBigCardTitle ?? "What's included in every white-label deal";

  const highlights = content?.whiteLabelBigCardSubTitle
    ? content.whiteLabelBigCardSubTitle
        .split("\n")
        .map((s) => s.trim())
        .filter(Boolean)
    : DEFAULT_HIGHLIGHTS;

  const bigCardDescription =
    content?.whiteLabelBigCardDescription ??
    "Interested in launching your branded LMS? Our team is ready to walk you through a personalized demo.";

  const bigCardButtonText = content?.whiteLabelBigCardButtonText ?? "Talk to Sales";
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex-1">
            <h3 className="text-xl font-semibold text-white mb-6">{bigCardTitle}</h3>
